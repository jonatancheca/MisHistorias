...
const suggestions = computed(() => [
  ...
  ...catalog.prompts.flatMap(item => item.tags)
].filter(tag => tagKey(tag) !== 'neutral'))
...
<template>
  <template v-if="settings.settings.swarmBaseUrl.trim()">
    <div class="settings-panel-content grid min-w-0 gap-6">
      <div class="flex flex-wrap justify-end gap-2">
        ...
        <button type="button" class="btn-primary" :disabled="busy || refreshing" @click="select(null)">
          Nuevo prompt
        </button>
      </div>
      ...
      <div class="grid min-w-0 gap-6 md:grid-cols-[240px_minmax(0,1fr)]">
        <ul class="grid h-fit min-w-0 gap-1">
          <li v-for="item in catalog.prompts" :key="item.id">
            <button
              type="button"
              class="w-full break-words rounded-lg px-3 py-2 text-left text-sm"
          ...
          <li v-if="!catalog.prompts.length" class="text-sm text-[var(--color-fg-muted)]">
            Sin prompts todavía.
          </li>
        </ul>
        ...
        <form class="grid min-w-0 gap-4" @submit.prevent="flushSave">
          ...
          <div class="flex min-h-10 flex-wrap items-center gap-3">
            <button type="button" class="btn-danger" :disabled="busy || !selectedId" @click="remove">
              Borrar
            </button>
  ...
  <p v-else>
    Indica primero la <a href="#swarmui" class="underline">URL de SwarmUI</a>.
  </p>
</template>
