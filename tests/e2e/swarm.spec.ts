...
test('oculta funciones sin URL y permite configurar sin valor por defecto', async ({ page, data }) => {
  ...
  await expect(page.getByLabel('Prompt visual base')).toBeVisible()
  await expect(page.getByText('Configura la URL de SwarmUI')).toBeVisible()
  await expect(page.getByLabel('Preset SwarmUI')).toHaveCount(0)
...
test('mantiene catálogo y crea conjunto con semillas compartidas, etiquetas y transferencia JSON', async ({ page, data }) => {
  ...
  await prefixOnlyDialog.getByRole('button', { name: 'Cancelar', exact: true }).click()
  ...
  const imageCard = page.getByTestId('character-image-card').first()
  await imageCard.getByRole('button', { name: 'Ampliar imagen', exact: true }).click()
  const imageDialog = page.getByRole('dialog', { name: 'Imagen ampliada', exact: true })
  await expect(imageDialog.getByTestId('image-generation-metadata')).toHaveCount(0)
  await imageDialog.getByRole('button', { name: 'Mostrar metadatos de IA', exact: true }).click()
  await expect(imageDialog.getByTestId('image-generation-metadata')).toContainText('quality')
  await imageDialog.getByRole('button', { name: 'Ocultar metadatos de IA', exact: true }).click()
  await expect(imageDialog.getByTestId('image-generation-metadata')).toHaveCount(0)
  await imageDialog.getByRole('button', { name: 'Cerrar imagen', exact: true }).click()
