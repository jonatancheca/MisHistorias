...
test('migra v1 a v32 copiando personajes y dejando modo visual desactivado', () => {
  ...
  try {
    ...
    assert.equal(storage.health().schemaVersion, 32)
...
test('conserva backup y revierte la base original si falla la migración', () => {
  ...
  try {
    assert.throws(
      ...
      /Falló la migración SQLite v3 a v32\. Backup:/
    )
...
test('no inicia la migración si no puede crear el backup', () => {
  ...
  try {
    assert.throws(
      () => new MisHistoriasStorage(path),
      /No se pudo crear el backup previo de SQLite\. Migración v3 a v32 no iniciada\./
    )
    const source = new DatabaseSync(path, { readOnly: true })
    try {
      assert.equal(
...
test('crea, lista y restaura backups manuales conservando todos los ámbitos', () => {
  ...
  try {
    ...
    assert.equal(backup.schemaVersion, 32)
...
test('migra v30, conserva mensajes antiguos y restaura diagnósticos sin mezclar ámbitos', () => {
  withStorage((storage, path) => {
    ...
    try {
      ...
      assert.equal(schema.prepare('PRAGMA user_version').get()?.user_version, 32)
      schema.close()
